(function(root, factory) {
	...
	function LexiconRainbow() {
		...
		this.append = function(ieScale) {
			...
			def.append("filter")
				...
				.attr("id", ID + "_gaussianBlur")
				.attr("stdDeviation", "0")
				.attr("in", "SourceGraphic");
			//clipPath for image
			def.append("clipPath")
				.attr("id", ID + "_clipperImage")
				.append("circle")
				.attr("cx", 0)
				.attr("cy", 0)
				.attr("r", min(0.8 * 0.25 * attrW, 0.6 * 0.8 * attrH) / 2);
			def.append("clipPath")
				.attr("id", ID + "_clipperRect")
				.append("rect")
				.attr("x", -50)
				.attr("y", 0)
				.attr("width", attrW * 0.75 + 50)
				.attr("height", 0.2 * attrH)
				.attr("rx", min(attrW, attrH) / 50)
				.attr("ry", min(attrW, attrH) / 50);
			def.append("clipPath")
				.attr("id", ID + "_clipperLeftPanel")
				.append("rect")
				.attr("x", 0)
				.attr("y", 0)
				.attr("width", attrW)
				...
				.attr("rx", min(attrW, attrH) / 50)
				.attr("ry", min(attrW, attrH) / 50);
			ordinalRect = d3.select("#" + ID + "_extras")
				.append("clipPath")
				.attr("id", ID + "_clipperOrdinalRect")
				.append("rect")
				.attr("x", 0)
				.attr("y", 0)
				.attr("width", 0)
				.attr("height", 0.9 * attrH)
				.attr("rx", min(attrW, attrH) / 30)
				.attr("ry", min(attrW, attrH) / 30); //height is bit bigger to make bottom of the rects flat
			//neon
			var neon = def.append("filter")
				.attr("id", "NeonGlow")
				.attr("x", 0)
				.attr("y", 0)
				.attr("width", 1)
				.attr("height", 1);
			neon.append("feColorMatrix")
				.attr("in", "SourceGraphic")
				.attr("type", "matrix")
				.attr("values", "1 0 0 0 0.5, 0 1 0 0 0.5, 0 0 1 0 0.5, 0 0 0 16 -10")
				.attr("result", "feColorMatrixOut");
			neon.append("feGaussianBlur")
				.attr("in", "feColorMatrixOut")
				.attr("stdDeviation", 0.5)
				.attr("result", "feGaussianBlurOut");
			neon.append("feBlend")
				.attr("in", "SourceGraphic")
				.attr("in2", "feGaussianBlurOut")
				.attr("result", "feBlendOut")
				.attr("mode", "screen");
			//***Defs***

			viewportBackground = d3.select("#" + ID)
				.append("g")
				.attr("clip-path", "url(#" + ID + "_clipperLeftPanel" + ")");
			viewport = d3.select("#" + ID)
				.append("g"); //items in here
			viewportOrdinal = viewport.append("g");
			viewportLinear = viewport.append("g");
			viewportFront = d3.select("#" + ID)
				.append("g"); //overlay items here
		...
		this.render = function(scale) {
			...
			viewportTemporary = viewportFront.append("g")
				.attr("id", ID + "_temporary");
			gStackObj = {};
		...
		this.renderOrdinal = function(ordinalData, container, scale) {
			var width = 0.70 * attrW / Object.keys(_input_.ordinal[ordinalID].categories)
				.length,
			...
			//enter
			var selection = container.selectAll("." + ID + "_ordinalBoxes")
				.data(ordinalData, function(d, i) {
					return d
				});
			var merged = selection
				...
				.each(function(d, i) {
					...
					thisG
						...
						.attr("stroke-width", 0)
						.attr("fill", returnColor("ordinal", cState, undefined, cColors,
							undefined, i, d)) //necessary to prevent initially black curves during transition
				...
				.on("touchstart", function(d, i) {
					this.touchCount = this.touchCount === undefined ? 0 : ++this.touchCount %
						2;
					if (!this.touchCount) {
						_this_.onpickStart(enableOnPick, d3.event, this, attrW, attrH, width,
							scaleInTransition,
							function() {
								highlightSolidCurve(container, i, d, undefined)
									.__lexiconExtend__()
				...
				.on("touchend", function(d, i) {
					if (this.touchCount) {
						_this_.onpickEnd(enableOnPick, d3.event, this, attrW, attrH, width,
							scaleInTransition,
							function() {
								highlightSolidCurve(container, i, d, undefined)
									.__lexiconShrink__()
			...
			merged
				...
				.attr("fill", function(d, i) {
					return returnColor("ordinal", cState, undefined, cColors, undefined,
						this.__lexiconIndex__, d)
				})
				.attr("width", width)
				.delay(0)
				.duration(500);
				...
				case 3:
					...
					merged.selectAll("rect.partition")
						...
						.attr("fill", function(d) {
							return returnColor("partition", partition === "color" ? cState :
								false, undefined, cColors, undefined, this.__lexiconIndex__, d)
						})
						.attr("width", width)
						.attr("fill-opacity", 0.6)
						.delay(0)
			...
			function makePartition(thisG, d, i) {
				return thisG
					...
					.attr("stroke-width", 0)
					.property("__lexiconIndex__", i)
					.attr("fill", returnColor("partition", partition === "color" ? cState :
						false, undefined, cColors, undefined, i, d)) //necessary to prevent initially black curves during transition
					.attr("fill-opacity", 0)
		...
		this.renderSolidCurve = function(ordinalData, container, scale) {
			var selection = container.selectAll("." + ID + "_ordinalSolidCurves")
				.data(ordinalData, function(d, i) {
					return d
				}),
				cOrdinal = _input_.ordinal[ordinalID], //current ordinal
				...
				sort = linear$ID$.sort,
				doExistSort = !!sort,
				offset = doExistMode ? typeof mode === "object" ? mode[1] : 0 :
				undefined,
				mode = doExistMode ? typeof mode === "object" ? mode[0] : mode :
				undefined,
				/* Modify#1 taken from renderCurves */
				gMode = linear$ID$.gMode,
				gModeBool = !!gMode,
				gStackContext = 0;
			gModeCondition = +gModeBool + ((gMode === "justify") << 1) + (doExistMode <<
				2);
			...
			for (var u = container.node()
					.querySelectorAll("." + ID + "_ordinalSolidCurves"), j = 0, k = u.length; j <
				k; ++j) {
				if (u[j].querySelectorAll("path:not([toBeRemoved])")
					.length !== 0) {
					return
				}
			}
			...
			function renderCurves(d, i) {
				var refPath = undefined,
					...
					categories$d$intervals = categories$d$intervals ?
					categories$d$intervals : (isRef = true, refPath = _input_.linear[+
						categories$d$].categories[d], refPath.intervals),
					names = doExistNames ?
					typeof categories$d$names === "string" ? [categories$d$names] :
					categories$d$names :
					isRef ? typeof refPath.names === "string" ? [refPath.names] : refPath.names :
					undefined,
					namesBool = !!names,
					isObjIntervals = typeof categories$d$intervals === "object",
					intervals = isObjIntervals ? categories$d$intervals.slice() : [
						categories$d$intervals
					],
					intervals = coerceToNumber(intervals), //coerce all to number
					intervals = doExistSort ? sortInterval(sort, indexPlaceHolder, names,
						intervals) : intervals,
					...
					stackingContextOrdinal = cOrdinalModeValue === "stackEqual" ?
					stackingContext.map(function(d, i) {
						return [i * stackingStep, (i + 1) * stackingStep]
					}) : restackOrdinal(stackingContext, stackingStep, stackingSpan),
					stackingContextLinear = mode === "intervalize" ? intervals.map(function(
						d, i, a) {
						return [0, typeof d === "object" ? (d[1] - d[0]) : d]
					}) : stackingContext,
					stackingSpanLinear = mode === "intervalize" ? max.apply(this, Array.prototype
						.concat.apply([], stackingContextLinear)
						.map(function(d) {
							return +d
						})) : stackingSpan;
				doExistMode ? offset ? intervals = stackingContextLinear.map(function(d,
					i) {
				...
				switch (gModeCondition) {
					...
					case 1: //gMode, stack, no mode,
						...
						gStackContext += max.apply(this, Array.prototype.concat.apply([],
								intervals)
							.map(function(d) {
								return +d
							}));
						intervals = intervals.map(function(u, w) {
							return typeof u === "object" ? [u[0] + gStackObj[d], u[1] +
								gStackObj[d]
							] : u + gStackObj[d]
						});
						break;
				...
				var selection = d3.select(this)
					.selectAll("path:not([toBeRemoved])." + ID + "_entered")
					.data(intervals, function(dd, ii) {
				...
				selection
					...
					.attr("fill", function(dd, ii) {
						return returnColor("linear", cState, isObjColors, cColors,
							colorsLength, i, d, ii, dd)
					...
					.on("mouseover", function(dd, ii) {
						...
						clonedNode.setAttribute("stroke", returnColor("stroke", cState,
							isObjColors, cColors, colorsLength, i, d, ii, dd));
						...
						clonedNode.gesture = function(event) {
							...
							cleanHover();
							handleEvent({
								name: names ? isRef && doExistSort ? names[indexPlaceHolder[ii]] :
									names[ii] : names,
								item: dd,
								parent: categories$d$,
					...
					.attr("d", function(dd, ii) {
						return drawSolidCurve(i, dd, undefined, scale, (cOrdinalMode ?
							stackingContextOrdinal[ii] : undefined), stackingSpan)
					})
					.attr("fill", function(dd, ii) {
						return returnColor("linear", cState, isObjColors, cColors,
							colorsLength, i, d, ii, dd)
		...
		function renderLeftPanel(containerOrdinal, containerLinear, containerImage) {
			function getIndexOf(node) {
				...
				for (var pNode = node.parentNode, cNodes = pNode.childNodes, i = 0; i <
					length; ++i) {
					if (cNodes[i] === node) {
						break;
					}
			...
			var obj = {
				...
				hasImage: function() {
					return Array.prototype.some.call(this.subject.node()
						.childNodes,
						function(d, i) {
			...
			var topRightRect = containerOrdinal.append("rect")
				.classed("GUI", true)
				.attr("x", 0)
				.attr("y", 0)
				.attr("width", attrW * 0.75)
				.attr("height", 0.2 * attrH)
				.attr("fill", guiColor)
				.attr("fill-opacity", guiIsOn ? 0.8 : 0)
				.style("visibility", guiIsOn ? "visible" : "hidden")
				.attr("transform", "translate(" + (0.25 * attrW) + "," + (0) + ")")
				.attr("clip-path", "url(#" + ID + "_clipperRect)")
				.node();
			var botRightRect = containerLinear.append("rect")
				.classed("GUI", true)
				.attr("x", 0)
				.attr("y", 0)
				.attr("width", attrW * 0.75)
				...
				.attr("fill-opacity", guiIsOn ? 0.8 : 0)
				.style("visibility", guiIsOn ? "visible" : "hidden")
				.attr("transform", "translate(" + (0.25 * attrW) + "," + (0.8 * attrH) +
					")")
				.attr("clip-path", "url(#" + ID + "_clipperRect)")
				.node();
			...
			var ordinalG = containerOrdinal.append("g")
				.classed("GUI", true)
				.attr("fill-opacity", guiIsOn ? 0.8 : 0)
				.style("visibility", guiIsOn ? "visible" : "hidden");
			var linearG = containerLinear.append("g")
				.classed("GUI", true)
				.attr("fill-opacity", guiIsOn ? 0.8 : 0)
				.style("visibility", guiIsOn ? "visible" : "hidden");
			var imageG = containerImage.append("g")
				.classed("GUI", true)
				.attr("fill-opacity", guiIsOn ? 0.8 : 0)
				.style("visibility", guiIsOn ? "visible" : "hidden");
			...
			_this_.linearG.update = function(count) {
				...
				_this_.changeScale(_input_.linear[linearID].domain, _input_.linear[
					linearID].format);
			...
			ordinalG.append("rect")
				.classed("GUI", true)
				.attr("x", 0)
				.attr("y", 0)
				.attr("width", attrW * 0.25)
				.attr("height", attrH * 0.2)
				.attr("fill", guiColor);
			linearG.append("rect")
				.classed("GUI", true)
				.attr("x", 0)
			...
			imageG.append("rect")
				...
				.attr("y", 0.2 * attrH)
				.attr("width", attrW * 0.25)
				.attr("height", attrH * 0.6)
				.attr("fill", guiColor);
			ordinalG.append("text")
				.text(_input_.ordinal[0].name)
				.attr("x", 0)
				.attr("dx", 0.125 * attrW)
				.attr("y", 0)
				.attr("dy", 0.15 * attrH)
				...
				.attr("font-size", 0.2 * attrH * 0.4)
				.attr("fill", _tagColors_[1])
				.attr("stroke", _tagColors_[1])
				.attr("stroke-width", 0)
				.style("cursor", "pointer");
			linearG.append("text")
				.text(_input_.linear[0].name)
				.attr("x", 0)
				.attr("dx", 0.125 * attrW)
				.attr("y", 0.8 * attrH)
				.attr("dy", 0.15 * attrH)
				...
				.style("cursor", "pointer");
			imageG.append("image")
				.attr("xlink:href", _input_.linear[0].glyph)
				.attr("alt", "droplet")
				.attr("x", -0.4 * 0.25 * attrW)
				.attr("y", -0.4 * 0.6 * attrH)
				.attr("height", 0.8 * 0.6 * attrH)
				.attr("width", 0.8 * 0.25 * attrW)
				.style("opacity", 0.7)
				.attr("clip-path", "url(#" + ID + "_clipperImage)")
				.attr("transform", "translate(" + (0.5 * 0.25 * attrW) + "," + (0.5 * 0.6 *
					attrH + 0.2 * attrH) + ")");
			//slider
			var slider = imageG.append("g")
				.attr("transform", "translate(0," + attrH * 0.2 + ")");
			slider.node()
				.ordinal = Array.prototype.concat.call(["__mock__", "__mock__"], _input_.ordinal
					.map(function(d, i) {
						return d.name
					}), ["__mock__", "__mock__"]);
			slider.node()
				.linear = Array.prototype.concat.call(["__mock__", "__mock__"], _input_.linear
					.map(function(d, i) {
						return d.name
					}), ["__mock__", "__mock__"]);
			slider.node()
			...
			slider.node()
				.__ready__ = true;
			slider.node()
				.render = function(type, count, offset) {
					if (!this.__ready__) {
						return
					}
					var that = this;
					window.requestAnimationFrame(function() {
						...
						count = count || 0;
						offset = offset || 0;
						var current = max(0, min(_input_[type].length - 1, round(count +
							offset)));
						...
						if (!that.__on__) {
							imageG.select("image")
								...
								.tween("blur", function() {
									...
									return function(t) {
										document.getElementById(ID + "_gaussianBlur")
											.setAttribute("stdDeviation", interpolator(t))
									}
								})
								.delay(0)
								.duration(1500);
							...
							rect.node()
								.__count___ = undefined;
							rect.node()
								.__dCount__ = undefined;
							rect.node()
								.__timer__ = new(function(arg) {
							...
							function dragFunc() {
								...
								var rawCount = pos / this.__yMax__ * (this.__yMax__ + this.__knobLengthSurplus__) /
									(attrH * 0.6) * length || 0; //when this.__yMax__ is 0, that when there is only 1 item __count__ and __dCount__ will become NaN because of division by 0, logical or necessary
								var count = floor(rawCount);
								var offset = rawCount - count;
								this.__timer__.elapsed >= 350 ? (this.__count__ = count, this.__timer__
									.startTime = undefined) : void(0);
								this.__dCount__ = count - this.__count__;
								d3.select(this)
									.attr("y", pos);
								slider.node()
									.render(type, count, offset);
							...
							function dragEnd() {
								...
								d3.select(this)
									...
									.tween("friction", function() {
										...
										var interpolator = d3.interpolate(this.__count__ + this.__dCount__,
											this.__count__ + this.__dCount__ * 2);
										var thisNode = this;
										return function(t) {
											var real = interpolator(t);
											var whole = floor(real);
											thisNode.setAttribute("y", max(0, min(thisNode.__yMax__, real /
												countMax * thisNode.__yMax__)));
											thisNode.__count__ = max(0, min(whole, length - 5));
											slider.node()
												.render(type, thisNode.__count__, real - whole);
										}
									})
						...
						var selection = slider.selectAll("." + ID + "_sliderTexts")
							.data(that[type].slice(count, count + 5));
						selection
							.enter()
							.append("text")
							.attr("font-family", "advent-pro")
							...
							.attr("fill", function(d, i) { /*console.log("i:"+i+",offset:"+offset+",current:"+current+",count:"+count);*/
								var corrected = offset !== 0.5 ? round(i - offset) : floor(i -
									offset);
							...
							.attr("transform", function(d, i) {
								return "translate(" + (0.125 * attrW) + "," + ((i + 1 - offset) *
										attrH * 0.12) + ") scale(1," + (1 - 0.4 * abs(2 - i + offset)) +
									")"
							});
						selection
							.exit()
							.remove();
						_this_[type + "G"].counter() !== current ? _this_.update(type,
							current) : void(0);
			...
			slider.node()
				.remove = function() {
					...
					slider.select("rect")
						.remove();
					slider.selectAll("." + ID + "_sliderTexts")
						.classed(ID + "_sliderTexts", false)
						.transition()
						.attr("fill-opacity", 0)
						.on("end", function() {
							d3.select(this)
			...
			d3.drag()
				.on("start", function() {
					...
					this.__rawCount__ = (+rect.getAttribute("y")) / yMax * (yMax +
						knobLengthSurplus) / (attrH * 0.6) * length || 0;
				})
				.on("drag", function dragFunc() {
					this.__rawCount__ = max(0, min(this.__rawCount__ += d3.event.dy / attrH /
						0.12, this.__length__ - 5));
					var pos = this.__rawCount__ * (attrH * 0.6) * this.__yMax__ / this.__length__ /
						(this.__yMax__ + this.__knobLengthSurplus__) || 0;
					var count = floor(this.__rawCount__);
					var offset = this.__rawCount__ - count;
					this.querySelector("rect")
						.setAttribute("y", pos);
		...
		function renderScale(container) {
			//##############################################################SCALE###################################################################################
			scaleMainBot = d3.scaleLinear()
				.domain(_input_.linear[0].domain)
				.range([0, 0.70 * attrW]);
			axisMainBot = d3.axisBottom()
				.scale(scaleMainBot)
				.ticks(5)
				.tickSize(-0.025 * attrH, 0)
				.tickPadding(4)
				.tickFormat(_input_.linear[0].format ? d3.format(_input_.linear[0].format) :
					null);

			var axisG = container.append("g")
				.attr("id", ID + "_lexMainAxisBot")
				.attr("class", "global_lexMainAxes")
				.attr("class", "global_lexScaleAxes")
				.attr("transform", "translate(" + (0.25 * attrW - 0.5) + "," + (0.8 *
					attrH + 0.025 * attrH) + ")")
				.call(axisMainBot);
			//##############################################################SCALE###################################################################################
			_this_.changeScale = function(domain, format) {
				...
				container.select("#" + ID + "_lexMainAxisBot")
					.transition()
		...
		function drawSolidCurve(order, domain, side, scale, stackingContext,
			stackingSpan) {
			stackingSpan = stackingSpan || 1;
			var currentScale = scale ? d3.scaleLinear()
				.domain(scale)
				.range([0, 0.70 * attrW]) : scaleMainBot;
			var epsilon = dispersion * attrW;
			var delta = 0.1 * 0.6 * attrH;
			var offset = 0.25 * attrW;
			var length = Object.keys(_input_.ordinal[ordinalID].categories)
				.length;
			var tabLength = attrW * 0.70 / length;
			var start1 = stackingContext ? [tabLength * (order + stackingContext[0] /
				stackingSpan) + offset, 0.2 * attrH] : [order * tabLength + offset, 0.2 *
				attrH
			];
			var start2 = stackingContext ? [tabLength * (order + stackingContext[1] /
				stackingSpan) + offset, 0.2 * attrH] : [(order + 1) * tabLength + offset,
				0.2 * attrH
			];
			var end1 = [typeof domain === "object" ? currentScale(+domain[0]) + offset :
				currentScale(+domain) + offset - epsilon, 0.8 * attrH
			];
			var end2 = [typeof domain === "object" ? currentScale(+domain[1]) + offset :
				currentScale(+domain) + offset + epsilon, 0.8 * attrH
			];
		...
		function highlightSolidCurve(container, order, data, scale) {
			...
			var operateOnIntervals = operateOn.intervals,
				operateOnIntervals = operateOnIntervals ? operateOnIntervals : (isRef =
					true, refPath = _input_.linear[+operateOn].categories[data], refPath.intervals
				);
			...
			var mode = linear$ID$.mode,
				...
				isRef ? typeof refPath.names === "string" ? [refPath.names] : refPath.names :
				undefined,
				converted = typeof operateOnIntervals === "object" ? operateOnIntervals.slice() :
				[operateOnIntervals],
				converted = coerceToNumber(converted), //coerce all to number
				converted = doExistSort ? sortInterval(sort, undefined, names, converted) :
				converted,
				cOrdinal = _input_.ordinal[ordinalID], //current ordinal
				cColors = cOrdinal.colors, //current colors
				cState = "colors" in cOrdinal, //current color state
				...
				stackingContextLinear = mode === "intervalize" ? converted.map(function(d,
					i, a) {
					return [0, typeof d === "object" ? (d[1] - d[0]) : d]
				}) : stackingContext,
				stackingSpanLinear = mode === "intervalize" ? max.apply(this, Array.prototype
			...
			switch (gModeCondition) {
				...
				case 1: //gMode, stack, no mode,
					converted = converted.map(function(u, w) {
						return typeof u === "object" ? [u[0] + gStackObj[data], u[1] +
							gStackObj[data]
						] : u + gStackObj[data]
					});
					break;
			...
			var selection = viewportTemporary.selectAll("." + ID +
					"_ordinalSolidHighlights_" + dataU)
				.data(expanded)
				.enter()
				.append("path")
				.attr("fill", "none")
				.attr("stroke", function() {
					return returnColor("stroke", cState, isObjColors, cColors, colorsLength,
						order, data)
				})
				...
				.each(function(dd, ii) {
					this.extend = extend.bind(this, order, dd, (cOrdinalMode ?
						stackingContextOrdinal[floor(ii / 2)] : undefined), stackingSpan);
			...
			function extend(order, domainExpanded, $stackingContext, $stackingSpan) {
				if (!this.__pointData__) {
					...
					node.setAttribute("d", drawSolidCurve(order, domainExpanded[0],
						domainExpanded[1], scale, $stackingContext, $stackingSpan));
					var totalLength = node.getTotalLength();
					this.__pointData__ = [];
					this.__pointData__.totalLength = totalLength;
					for (var i = 0, sampleD = 3, point = node.getPointAtLength(0); i *
						sampleD <= totalLength; ++i, point = node.getPointAtLength(i * sampleD)
					) {
						this.__pointData__.push([point.x, point.y])
					}
				}
				var overhang = floor(((this.hasAttribute("d") ? this.getTotalLength() : 0) /
					this.__pointData__.totalLength) * this.__pointData__.length) || 1;
				d3.select(this)
					...
					.attrTween("d", function() {
						...
						return (function(t) {
								return basis(this.__pointData__.slice(0, ceil(t * length + (1 - t) *
									overhang)))
							})
							.bind(this)
					})
			...
			function shrink(order, domainExpanded) {
				...
				var overhang = this.__pointData__.slice(0, floor((this.getTotalLength() /
					this.__pointData__.totalLength) * this.__pointData__.length) || 1);
			...
			function massShrink() {
				d3.selectAll("." + ID + "_ordinalSolidHighlights_" + dataU)
					.each(function() {
						this.shrink();
					})
			}
		...
		function sortObject(obj) {
			...
			function reduceObj(obj) {
				...
				for (var i = 0, total = 0; i < flattened.length; total += +flattened[i], ++
					i) {}
				return total / i;
			}
		}
			...
			*/
			switch (sortOpt) {
				case ">":
					return intervals
						.map(function(d, i) {
							return typeof d === "object" ? [max.apply(this, d) * base + i, i, d] :
								[d * base + i, i, d];
						})
						.sort(function(a, b) {
							return a[0] - b[0];
				...
				case "|>|":
					return intervals
						.map(function(d, i) {
							return typeof d === "object" ? [abs(d[1] - d[0]) * base + i, i, d] : [
								i, i, d
							];
						})
						.sort(function(a, b) {
		...
		this.unwarp = function(f) {
			var viewBox = d3.select("#" + ID)
				.attr("viewBox");
			var viewBoxFinal = viewBox.split(" ")
				.map(function(d, i) {
					return d * 100
				})
				.join(" ");
			d3.select("#" + ID)
				...
				.tween("unwarp", function() {
					...
					return function(t) {
						thisNode.setAttribute("viewBox", interpolator(t))
					}
				})
				.delay(0)
				.duration(3000);
		...
		function returnColor(whatToReturn, cState, isObjColors, cColors,
			cColorsLength, i, d, ii, dd) {
			isObjColors = isObjColors || (cState ? typeof cColors[d] === "object" :
				false);
			switch (whatToReturn) {
				case "ordinal":
		...
		function restackOrdinal(arr, step, span) {
			...
			for (var i = 0, o = 0; i < l; v = arr2[i], arr2[i] = [o, o + v * c], o +=
				v * c, ++i) {}
			for (var i = 0, j = span / o; i < l; v = arr2[i], arr2[i] = [v[0] * j, v[1] *
					j
				], ++i) {}
			return arr2;
		}
	...
	prt.shapeRendering = function(v) {
		if (arguments.length && !this.isAppended) {
			...
			throw new Error(
				"Cannot set shape-rendering after render method is called already");
		} else {
	...
	prt.update = function(type, current) {
		...
		internal.handleEvent(internal.input[type][current], "onrender" + type.charAt(
				0)
			.toUpperCase() + type.slice(1), null, true);
	...
	prt.GUI = function(bool, o) {
		switch (this.isAppended) {
			...
			case true:
				...
				d3.select("#" + this.lexID())
					.transition()
					.attr("viewBox", nViewBox)
					.delay(0)
					.duration(1000);
				return this;
		}
		...
		function guiOff(x, y, w, h) {
			return (x + 0.25 * w + (o ? o.x || 0 : 0)) + " " + (y + (o ? o.y || 0 : 0)) +
				" " + (x + w * 0.70 + (o ? o.w || 0 : 0)) + " " + (y + h + (o ? o.h || 0 :
					0))
		}

		function guiOn(x, y, w, h) {
			return (x + (o ? o.x || 0 : 0)) + " " + (y + (o ? o.y || 0 : 0)) + " " +
				(x + w + (o ? o.w || 0 : 0)) + " " + (y + h + (o ? o.h || 0 : 0))
		}

		function guiOffCanvas(w, h) {
			this.getNSet.canvas.attr("width", (w * 0.70 + (o ? o.w || 0 : 0)))
				.attr("height", (h + (o ? o.h || 0 : 0)))
		}
	...
	prt.onpickStart = function(enableOnPick, e, oNode, attrW, attrH, width,
		scaleInTransition, request, handle, input, linearID, d, i) {
		...
		if (d3.select(document.elementFromPoint(client.x + 1, client.y))
			.classed("partition")) {
			return
		}
		var thisG = d3.select(oNode);
		thisG.transition("draw")
			.ease(d3.easeLinear)
			...
			.delay(0)
			.duration(250);
		thisG.select("rect.main")
			.transition("extend")
			.ease(d3.easeLinear)
			.attr("height", 0.20 * attrH)
			.delay(0)
			.duration(250);
		thisG.select("rect.partition")
			.transition("pushDown")
			.ease(d3.easeLinear)
			.attr("transform", "translate(0," + (0.20 * attrH) + ")")
			.delay(0)
			.duration(250);
		enableOnPick !== "noLineAnim" ? window.requestAnimationFrame(request) :
			void(0);
		handle({
			name: d,
	...
	prt.onpickEnd = function(enableOnPick, e, oNode, attrW, attrH, width,
		scaleInTransition, request, handle, input, linearID, d, i) {
		//alert("mouseleave!");
		if (!enableOnPick || scaleInTransition) {
			return
