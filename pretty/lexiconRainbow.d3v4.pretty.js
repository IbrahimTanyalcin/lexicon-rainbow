	...
	function LexiconRainbow() {
		...
		this.renderOrdinal = function(ordinalData, container, scale) {
			...
			var merged = selection
				...
				.on("mouseenter", function(d, i) {
					...
					if (d3.select(document.elementFromPoint(d3.event.clientX + 1, d3.event.clientY))
						.classed("partition")) {
						return
					}
					var thisG = d3.select(this);
					...
					thisG.select("rect.partition")
						...
						.delay(0)
						.duration(250);
					scaleInTransition ? void(0) : window.requestAnimationFrame(function() {
		...
		this.renderSolidCurve = function(ordinalData, container, scale) {
			...
			for (var u = container.node()
					.querySelectorAll("." + ID + "_ordinalSolidCurves"), j = 0, k = u.length; j < k; ++j) {
			...
			function renderCurves(d, i) {
				var refPath = undefined,
					...
					stackingSpanLinear = mode === "intervalize" ? max.apply(this, Array.prototype.concat.apply([], stackingContextLinear)
						.map(function(d) {
							return +d
						})) : stackingSpan;
				...
				switch (gModeCondition) {
					...
					case 1: //gMode, stack, no mode,
						...
						gStackContext += max.apply(this, Array.prototype.concat.apply([], intervals)
							.map(function(d) {
								return +d
							}));
				...
				selection
					...
					.on("mouseover", function(dd, ii) {
						...
						handleEvent({
							...
							parent: categories$d$,
							index: isObjIntervals ? ii : undefined
						}, "onhighlight", /*d3.event.type*/ "mouseover");
					})
		...
		function renderLeftPanel(containerOrdinal, containerLinear, containerImage) {
			...
			imageG.append("image")
				...
				.attr("transform", "translate(" + (0.5 * 0.25 * attrW) + "," + (0.5 * 0.6 * attrH + 0.2 * attrH) + ")");
			//slider
			var slider = imageG.append("g")
				.attr("transform", "translate(0," + attrH * 0.2 + ")");
			slider.node()
				.ordinal = Array.prototype.concat.call(["__mock__", "__mock__"], _input_.ordinal.map(function(d, i) {
					return d.name
				}), ["__mock__", "__mock__"]);
			slider.node()
				.linear = Array.prototype.concat.call(["__mock__", "__mock__"], _input_.linear.map(function(d, i) {
					return d.name
				}), ["__mock__", "__mock__"]);
			slider.node()
				.__on__ = 0;
				...
				.render = function(type, count, offset) {
					...
					window.requestAnimationFrame(function() {
						...
						selection
							.exit()
							.remove();
						_this_[type + "G"].counter() !== current ? (handleEvent(_input_[type][current], "onrender" + type.charAt(0)
							.toUpperCase() + type.slice(1), null), _this_[type + "G"].update(current)) : void(0);
		...
		function renderScale(container) {
			...
			axisMainBot = d3.axisBottom()
				...
				.tickPadding(4)
				.tickFormat(_input_.linear[0].format ? d3.format(_input_.linear[0].format) : null);

			var axisG = container.append("g")
				.attr("id", ID + "_lexMainAxisBot")
				.attr("class", "global_lexMainAxes")
				.attr("class", "global_lexScaleAxes")
				.attr("transform", "translate(" + (0.25 * attrW - 0.5) + "," + (0.8 * attrH + 5) + ")")
				.call(axisMainBot);
			//##############################################################SCALE###################################################################################
		...
		function highlightSolidCurve(container, order, data, scale) {
			...
			var selection = viewportTemporary.selectAll("." + ID + "_ordinalSolidHighlights_" + dataU)
				.data(expanded)
				.enter()
				.append("path")
				.attr("fill", "none")
