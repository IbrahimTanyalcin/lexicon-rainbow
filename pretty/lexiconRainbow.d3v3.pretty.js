(function(root, factory) {
	...
	function LexiconRainbow() {
								...
								d3.select(this)
									...
									.tween("friction", function() {
										...
										return function(t) {
											...
											this.setAttribute("y", max(0, min(this.__yMax__, real /
												countMax * this.__yMax__)));
											this.__count__ = max(0, min(whole, length - 5));
											slider.node()
												.render(type, this.__count__, real - whole);
										}
									})
		...
		function renderScale(container) {
			//##############################################################SCALE###################################################################################
			scaleMainBot = d3.scale.linear()
				.domain(_input_.linear[0].domain)
				.range([0, 0.70 * attrW]);
			axisMainBot = d3.svg.axis()
				.scale(scaleMainBot)
				.orient("bottom")
				.ticks(5)
				.tickSize(-0.025 * attrH, 0)
				.tickPadding(6)
				.tickFormat(d3.format(_input_.linear[0].format || null));
		...
		function drawSolidCurve(order, domain, side, scale, stackingContext,
			...
			var currentScale = scale ? d3.scale.linear()
				.domain(scale)
		...
		function highlightSolidCurve(container, order, data, scale) {
			...
			function extend(order, domainExpanded, $stackingContext, $stackingSpan) {
				...
				var overhang = floor(((this.hasAttribute("d") ? this.getTotalLength() : 0) /
					this.__pointData__.totalLength) * this.__pointData__.length);
				d3.select(this)
